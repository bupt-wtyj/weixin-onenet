...
Page({
  ...
  getDatapoints: function () {
    return new Promise((resolve, reject) => {
      wx.request({
        url: `https://api.heclouds.com/devices/${devicesId}/datapoints?datastream_id=Temperature,Humidity,Soilhumidity,&limit=20`,
        /**t
         * 添加HTTP报文的请求头, 
        ...
        success: (res) => {
          ...
          resolve({
            ...
            Soilhumidity: response.data.datastreams[2].datapoints.reverse(),
  ...
  update: function (datapoints) {
    ...
    this.lineChart_Soilhumidity.updateData({
      categories: wheatherData.categories,
      series: [{
        name: 'Soilhumidity',
        data: wheatherData.Soilhumidity,
        format: (val, name) => val.toFixed(2)
      }],
    })
  ...
  convert: function (datapoints) {
    ...
    for (var i = 0; i < length; i++) {
      ...
      Soilhumidity.push(datapoints.Soilhumidity[i].value);
  ...
  firstDraw: function (datapoints) {
    ...
    //土壤湿度
  this.lineChart_Soilhumidity = new myCharts({
    canvasId: 'Soilhumidity',
    type: 'line',
    categories: wheatherData.categories,
    animation: false,
    background: '#f5f5f5',
